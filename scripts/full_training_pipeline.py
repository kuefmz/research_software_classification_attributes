#!/usr/bin/env python3
"""Run local ML baselines on the full SOMEF-enriched dataset.

The input is the complete saved ``data/final/combined_all_with_somef.jsonl``
dataset. No external enrichment, downloads, or remote model calls are made.
"""
...
ANALYSIS_STAGE = "final_full_dataset"
RANDOM_SEED = config.RANDOM_SEED
INPUT_DATASET = config.DATA_DIR / "final_edam_top_level" / "combined_all_with_somef.jsonl"
RESULTS_DIR = config.RESULTS_DIR / "training"
SPLITS_DIR = config.SPLITS_DIR / "full_dataset"
LOG_PATH = RESULTS_DIR / "full_training_pipeline.log"
...
RESULTS_XLSX = RESULTS_DIR / "full_training_results.xlsx"
...
RUN_LLMS = False
RUN_LINEAR_BASELINE_ONLY = False
...
LOGGER = logging.getLogger("full_training_pipeline")
...
def save_run_metadata() -> None:
    """Save constants that define this final training run."""
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
        ...
        json.dumps(
            {
                ...
                "run_llms": RUN_LLMS,
                "run_linear_baseline_only": RUN_LINEAR_BASELINE_ONLY,
            },
            indent=2,
            sort_keys=True,
        ),
...
def load_records() -> list[dict[str, Any]]:
    """Load the full SOMEF-enriched dataset generated by the local pipeline."""
    if not INPUT_DATASET.exists():
        raise FileNotFoundError(f"Run the local data preparation/SOMEF pipeline first: {INPUT_DATASET}")
    records = list(read_jsonl(INPUT_DATASET))
    LOGGER.info("Loaded full SOMEF-enriched records: %s", f"{len(records):,}")
    return records
...
def create_or_load_split(records: list[dict], dataset_id: str, label_setting: str, skipped: list[dict]) -> dict[str, list[int]]:
    ...
    if label_setting == "single":
        ...
        skip(skipped, "iterative multi-label stratification not available in this local script; using deterministic random split", dataset_name=dataset_id)
    split = {
...
def model_specs(label_setting: str) -> list[dict[str, Any]]:
    """Return model specifications for currently enabled local baselines."""
    specs: list[dict[str, Any]] = []
    if not RUN_LINEAR_BASELINE_ONLY:
        specs.append(
            {
...
def run_placeholder_sections(skipped: list[dict]) -> list[dict]:
    ...
    for model_family, enabled in [
        ...
        row = {
            ...
            "enabled": enabled,
            "note": "Placeholder prepared. Disabled by default for the local final script to avoid heavy downloads/training.",
        }
        metadata.append(row)
        if not enabled:
...
def main() -> None:
    ...
    for output_path in [SUMMARY_CSV, PER_CLASS_CSV, SKIPPED_CSV, PREDICTIONS_JSONL, RESULTS_XLSX]:
        ...
        skipped.append(
            {
                "analysis_stage": ANALYSIS_STAGE,
                "timestamp": timestamp(),
                "reason": "xgboost disabled by RUN_XGBOOST_IF_AVAILABLE for local final run",
                "model_name": "tfidf_xgboost",
            }
        )
    if RUN_LINEAR_BASELINE_ONLY:
        skipped.extend(
            [
                {
                    "analysis_stage": ANALYSIS_STAGE,
                    "timestamp": timestamp(),
                    "reason": "disabled for tractable full-dataset final run; linear TF-IDF baseline evaluated",
                    "model_name": "tfidf_logistic_regression",
                },
                {
                    "analysis_stage": ANALYSIS_STAGE,
    ...
    for dataset_id, dataset_records in datasets.items():
        ...
        folds = create_or_load_folds(dataset_records, dataset_id, label_setting, skipped) if RUN_CROSS_VALIDATION else []
        if not RUN_CROSS_VALIDATION:
            skip(skipped, "cross-validation disabled for full-dataset runtime; deterministic train/test split still evaluated", dataset_name=dataset_id)
    ...
    LOGGER.info("Full dataset training complete. Results written to %s", RESULTS_DIR)
