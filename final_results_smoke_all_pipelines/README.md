...
`openai_zero_shot` and `openai_few_shot` use the source-controlled prompt in `prompts/openai_research_software_classifier.md`, copied into `prompt_templates/` for every run. The API is never called unless `--enable-openai` is passed. Responses are cached in `openai_cache/`, full request/assistant conversations are saved in `openai_conversations/`, and train/test predictions include the OpenAI model ID, prompt hash, cache key, conversation path, confidence, abstention flag, evidence snippets, and raw JSON response text.
...
| `openai_conversations/*.json` | Full OpenAI system/user/assistant conversations for audit and reuse. |
